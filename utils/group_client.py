...
class GroupClient:
    ...
    def set_group_server_distribute(self, group_id: str, enabled: bool) -> dict:
        payload = {"group_id": group_id, "user_id": self.app.my_pub_hex, "enabled": enabled}
        r = requests.post(f"{self.app.SERVER_URL}/groups/server_distribute/set", json=payload, verify=self.app.SERVER_CERT, timeout=10)
        r.raise_for_status()
        return r.json()

    def set_group_server_store_history(self, group_id: str, enabled: bool) -> dict:
        payload = {"group_id": group_id, "user_id": self.app.my_pub_hex, "enabled": enabled}
        r = requests.post(f"{self.app.SERVER_URL}/groups/server_store_history/set", json=payload, verify=self.app.SERVER_CERT, timeout=10)
        r.raise_for_status()
        return r.json()
